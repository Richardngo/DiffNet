...
if __name__ == "__main__":
    ...
    data_dir = f"{RAW_DIR}/u_{MIN_USER_REVIEWS}_i_{MIN_ITEM_REVIEWS}_f_{MIN_USER_FRIENDS}"
    out_dir = f"{data_dir}/out_score"
    ...
    with open(train_ratings_file, "w") as f1, open(train_ratings_file_graphrec, "w") as f2:
        for review in reviews[:train_count]:
            ...
            if not include_zero_ratings and rating == 0:
                continue
            f1.write(f'{users_map[review["user_id"]]},{businesses_map[review["business_id"]]},{rating}\n')
            f2.write(f'{users_map[review["user_id"]]},{businesses_map[review["business_id"]]},{int(review["stars"])}\n')
    ...
    with open(validation_ratings_file, "w") as f:
        for review in reviews[train_count : train_count + validation_count]:
            ...
            if not include_zero_ratings and rating == 0:
                continue
            f.write(f'{users_map[review["user_id"]]},{businesses_map[review["business_id"]]},{rating}\n')
